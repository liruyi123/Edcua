...
<div class="linevideo" style="overflow-x:hidden" id="div1">
    ...
    <span class="taskspan"><span class="ts">{{$video['cata_name']}}</span>&nbsp;&nbsp;<b class="tasktit">{{$video['cata_text']}}</b></span>
    <div style="width:100%;margin-top:20px;">
        <video width="auto" id="example_video_1" src="{{$video['video']}}" class="video-js vjs-default-skin  vjs-big-play-centered vvi video " controls preload="none"  poster="{{$data[0]['path']}}" data-setup="{}"><!--poster是视频未播放前的展示图片-->
                    ...
                    @foreach($arr as $k => $v)
                        ...
                        @foreach($v['son'] as $ke => $va)
                            ...
                            @foreach($va['son'] as $key => $val)
                                <a href="javascript:;" class="gain" cata_id="{{$val['cata_id']}}"><dd><i class="forwa nn"></i><strong class="cataloglink">{{$val['cata_name']}}：{{$val['cata_text']}}</strong></dd></a>
                            @endforeach
                        @endforeach
                    @endforeach
                </dl>
